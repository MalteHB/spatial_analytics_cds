...
cities_sf4326 <- cities_sf
st_crs(cities_sf4326) <- 4326
...
### Answer:
The measurement units are meters as this is the default length metric.
...
st_crs(cities_sf3035)$units # Still meters :-)
...
tm_shape(ten_buffers)  +
  ...
  tm_dots(col = "blue", 
             size = 0.1) +
  tm_scale_bar(breaks = c(5, 10, 20, 50),
               text.size = 10,
...
Heaven on earth, the place of lasagna and pizza, I-T-A-L-Y <3
...
# Create a convex hull around the inscription's points dissolved into a MULTIPOINT
...
The ratio is 1/1 since there are 11 inscriptions in both urban and rural areas.
...
length(unique(unlist(clist)))
which(!unique(unlist(clist)))
